...
describe('unit tests of DatePeriod', function () {
  it('should disallow invalid states', () => {
    let p = new DatePeriod(moment.utc('2018-01-01'), moment.utc('2018-01-02'))
    expect(() => { p.begin = p.end.add(1, 'day') }).to.throw(IllegalArgumentError)
    expect(() => { p.end = p.begin.add(-1, 'day') }).to.throw(IllegalArgumentError)
  })
  it('should disallow invalid begin granularity', () => {
    expect(() => new DatePeriod(moment.utc('2018-01-01T01:00Z'), moment.utc('2018-01-02'))).to.throw(IllegalArgumentError)
  })
  it('should disallow invalid end granularity', () => {
    expect(() => new DatePeriod(moment.utc('2018-01-01'), moment.utc('2018-01-02T01:00Z'))).to.throw(IllegalArgumentError)
  })
  it('should contain or not correctly', function () {
    let container = new DatePeriod(moment.utc('2018-01-01'), moment.utc('2018-01-03'))
    ...
    candidate = new DatePeriod(container.begin.add(1, 'day'), container.end.add(-1, 'day'))
    expect(container.containsPeriod(candidate)).to.be.true()
    ...
    expect(container.containsPeriod(new DatePeriod().withBegin(container.begin))).to.be.true()
    expect(container.containsPeriod(new DatePeriod().withBegin(container.begin.add(1, 'day')))).to.be.true()
    expect(container.containsPeriod(new DatePeriod().withBegin(container.begin.add(-1, 'day')))).to.be.false()

    container = new DatePeriod().withEnd(moment.utc('2018-01-01'))
    expect(container.containsPeriod(new DatePeriod().withEnd(container.end))).to.be.true()
    expect(container.containsPeriod(new DatePeriod().withEnd(container.end.add(1, 'day')))).to.be.false()
    expect(container.containsPeriod(new DatePeriod().withEnd(container.end.add(-1, 'day')))).to.be.true()
  ...
  it('should overlap or not correctly', function () {
    ...
    expect(a.overlaps(new DatePeriod().withBegin(a.begin.add(1, 'day')))).to.be.true()
    expect(a.overlaps(new DatePeriod().withBegin(a.begin.add(-1, 'day')))).to.be.true()

    a = new DatePeriod().withEnd(moment.utc('2018-01-01'))
    expect(a.overlaps(new DatePeriod().withEnd(a.end))).to.be.true()
    expect(a.overlaps(new DatePeriod().withEnd(a.end.add(1, 'day')))).to.be.true()
    expect(a.overlaps(new DatePeriod().withEnd(a.end.add(-1, 'day')))).to.be.true()
