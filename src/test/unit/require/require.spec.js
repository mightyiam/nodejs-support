...
describe('req', () => {
  it('should require all .js files', () => {
    let result = req.jsFilesIn(`${__dirname}/dirWithJsFiles`)
    ...
    result = req.jsFilesIn(`${__dirname}/dirWithDirsWithNamesThatLookLikeFiles`)
    expect(result.dir0).not.to.be.ok()
    expect(result.dir1).not.to.be.ok()
  })
  ...
  it('should require all .json files', () => {
    let result = req.jsonFilesIn(`${__dirname}/dirWithJsonFiles`)
    expect(result.file0).to.equal(0)
    expect(result.file1).to.equal(1)

    result = req.jsonFilesIn(`${__dirname}/dirWithDirsWithNamesThatLookLikeFiles`)
    expect(result.dir0).not.to.be.ok()
    expect(result.dir1).not.to.be.ok()
  })
})
